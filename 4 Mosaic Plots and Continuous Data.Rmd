...
+  The side-by-side plot is the easiest to see the conditional distributions of the second variable given each category of the first variable. Because on the side-by-side plot, we are basically looking at a group of bar charts, where each group is an airline, and within each group, number of sentiments are shown as bar charts. In this way, we can pick any airline (to condition on it), and find out the distribution of sentiments given this particular airline that we picked.

+  The porportional bar plot and the faceted graph provide the best balance of showing the marginal distribution of the first variable and the conditional distributions of the second variable. Because for the proportional bar plot, we can see very clearly the proportion of different kind of sentiment, with the bar for each airline. This is very good for us to identify the conditional distribution of sentiments. However, a trade-off is that in the proportional bar plot the marginal distribution is not very clearly shown, since everything has the same height. The faceted graph has made similar trade-offs. So these two grpahs are the most balanced ones between strengths and weaknesses.
